...
#output_TransitService <- reactive({
  ...
  Assumptions_transitservice <- rvs$Assumptions[rvs$Assumptions$table_no_ui == 2 & rvs$Assumptions$transit_category == 'Average Passenger-Mile Per Vehicle',] %>%
    select_if(~ any(!is.na(.)))
  
  Assumptions_transitservice2 <- rvs$Assumptions[rvs$Assumptions$table_no_ui == 2 & rvs$Assumptions$transit_category == 'On-Road Vehicle Fuel Economy',] %>%
    select_if(~ any(!is.na(.)))
  
  Assumptions_transitservice3 <- rvs$Assumptions[rvs$Assumptions$table_no_ui == 2 & rvs$Assumptions$transit_category == 'Revenue Mile Per Vehicle',] %>%
    select_if(~ any(!is.na(.))) %>%
    select(-table_no_ui,-unit,-category, -table, -transit_category)
  
  
  # get projects input
  Capital_Project_Inputs_transit_service <- rvs$Projects[rvs$Projects$table_no_ui == 4 | (rvs$Projects$table_no_ui == 3 & rvs$Projects$fuel_type == 'Electric'),] %>%
  ...
  transitservice_output <- Capital_Project_Inputs_transit_elec %>%
