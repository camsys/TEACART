...
# output_EVSE <- reactive({
  ...
  emrate_evse <- bind_rows( ### this runs pretty slow - needs to be rewritten Gui 1/22/24
  ...
  vmt_affected <-
    ...
    left_join(x = Stock, y = ., by = join_by(veh_supertype, year), relationship = "one-to-many") %>%
    summarize("VMT_affected" = sum(calculated_ports * MT_per_vehtype), .by = c("veh_supertype", "year"))
  
  evse_by_year_supertype <- Stock %>% 
