...
# output_bikped <- reactive({
  ...
  # preprocess the strategy parameters
  Assumptions <- rvs$Assumptions[rvs$Assumptions$table_no_ui == 1,] 
  
  # calculate the Displaced Auto mile/yr
  Bicycle_and_Pedestrian_base <- Bike_Ped %>% 
    mutate(annual_displaced_auto_miles = case_when(area_type == 'Core Urban' ~ -(daily_new_bicyclists*ssumptions$value[Assumptions$element == 'Core (>10,000 ppsm)']*
                                                                                   Assumptions$value[Assumptions$element == 'Bike'] +
                                                                                   daily_new_walkers * Assumptions$value[Assumptions$element == 'Walk']*
                                                                                   Assumptions$value[Assumptions$element == 'Core (>10,000 ppsm)'])*
                                                     Assumptions$value[Assumptions$element == 'annualization_factor'],
                                                  area_type == 'Urban' ~  -(daily_new_bicyclists*Assumptions$value[Assumptions$element == 'Urban (4,000 - 10,000 ppsm)']*
                                                                              Assumptions$value[Assumptions$element == 'Bike'] +
                                                                              daily_new_walkers * Assumptions$value[Assumptions$element == 'Walk']*
                                                                              Assumptions$value[Assumptions$element == 'Urban (4,000 - 10,000 ppsm)'])*
                                                    Assumptions$value[Assumptions$element == 'annualization_factor'],
                                                  area_type == 'Suburban' ~  -(daily_new_bicyclists*Assumptions$value[Assumptions$element == 'Suburban (500 - 4,000 ppsm)']*
                                                                                 Assumptions$value[Assumptions$element == 'Bike'] +
                                                                                 daily_new_walkers * Assumptions$value[Assumptions$element == 'Walk']*
                                                                                 Assumptions$value[Assumptions$element == 'Suburban (500 - 4,000 ppsm)'])*
                                                    Assumptions$value[Assumptions$element == 'annualization_factor'],
                                                  area_type == 'Rural' ~  -(daily_new_bicyclists*Assumptions$value[Assumptions$element == 'Rural (<500 ppsm)']*
                                                                              Assumptions$value[Assumptions$element == 'Bike'] +
                                                                              daily_new_walkers * Assumptions$value[Assumptions$element == 'Walk']*
                                                                              Assumptions$value[Assumptions$element == 'Rural (<500 ppsm)'])*
                                                    Assumptions$value[Assumptions$element == 'annualization_factor']
    )) 
  ## process the captial project Inputs for bicycle and Pedestrian
  ...
  Bicycle_and_Pedestrian_base <- Bicycle_and_Pedestrian_base %>% merge(.,captial_project_input_bkped, by = c('facility_type','area_type'), all = TRUE) %>% 
    mutate(total_vmt_change = annual_displaced_auto_miles * value,
           total_CO2_change = total_vmt_change * emrate/1000000,
           total_newtrips = facilitiesmile * (daily_new_bicyclists + daily_new_walkers),
           total_NOx_change = total_vmt_change * fuel_factorNox *emrate_by_tech/1000000,
           total_PM25_change = (total_vmt_change*fuel_factorPMe * emrate_by_tech + total_vmt_change * fuel_factorPMtb)/1000000
