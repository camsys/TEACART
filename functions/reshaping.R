...
reshaping <- function(user_data,
  ...
  return(updated_data)
    ...
    updated_data <- rvs[rvs$table_no_ui == tbl_no,colnames(rvs)[colnames(rvs) != 'value']]%>%
      left_join(select(modified_data, -var4), by = setNames(y_names,x_names)) # setNames(y,x) # SETH - curious what the setNames function is for? Right now this join error is throwing a type error
    #browser()
    print('check 3')
    return(updated_data)
  }else{
    ...
    updated_data <- rvs[rvs$table_no_ui == tbl_no,colnames(rvs)[colnames(rvs) != 'value']]%>%
      left_join(modified_data, by = setNames(y_names,x_names)) # setNames(y,x) # SETH - curious what the setNames function is for? Right now this join error is throwing a type error
    ##browser()
    print('NOTE: Data is being returned from reshaping function')
    return(updated_data)
  }
